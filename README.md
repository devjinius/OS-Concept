...
뭔가 많을 것이다. 화면에 보이는 것은 모두 프로세스다. 아무 작업을 실행하지 않았는데 수많은 프로세스가 실행되고 있다. 이 프로세스는 어떻게 **동시에** 실행되는 걸까? 운영체제가 프로세스를 동시에 실행할 수 있도록 한다. 어떤 순서로 프로세스를 실행할지 스케줄링하고 해당 프로세스가 CPU를 사용할 수 있게 한다.

### 동시-Simultaneous

**동시에**라는 표현을 짚고 넘어가자. 같을 동, 시간 시. 같은 시간에 라는 뜻이다. 일상생활에서 동시에 무엇을 한다는 표현을 자주 한다. 1시간 동안 밥을 먹으면서 동시에 코딩을 한다. 이상한 점을 못 느낀다. 그럼 그 1시간 중 1초를 생각해보자. 숟가락을 잡고 있거나 키보드에 손이 있거나 둘 중 하나일 것이다. 모든 순간에 동시 작업을 할 수 있나? 없다.  
동시라는 표현은, **일정 기간 동안** 여러 작업을 한다는 뜻이다. CPU는 한번에 한 작업밖에 할 수 없다. 그러나 CPU는 무지하게 빨라서, 백분의 일초면 많은 작업을 할 수 있다. 즉 우리가 미처 알아차리지 못하는 사이, 여러 프로세스가 실행되는 것이다.  

다른 예를 들어보자. 당신은 어느 회사에 신입사원으로 입사했다. 팀장님은 아주 무서운 분이시다. 업무시간에 딴짓하는 게 걸리면 무지하게 혼난다. 그러나 당신은 카카오톡도 해야하고 쇼핑도 해야한다. 팀장님이 안 볼때 신나게 카카오톡을 한다. 팀장님이 일어나면 냉큼 알트탭으로 일하는 척을 한다. 팀장님이 보기에 당신은 열심히 일하고 있다. 팀장님이 자리에 앉으면 다시 쇼핑을 시작한다. 팀장님이 일어나면 알탭. 앉으면 딴짓.  
당신은 동시에 여러 작업을 하고 있다. 업무, 카카오톡, 쇼핑. 3가지나 하고 있다. 그러나 팀장님이 보기에는 업무만 하고 있다. 왜? 팀장님 볼 때에는 업무만 하니까.  
이번에는 당신이 팀장, 컴퓨터는 핫산이라고 생각해보자. 당신은 유튜브를 틀어놓고 웹서핑을 하고 있다. 컴퓨터는 음악을 재생하기 위해 재빨리 당분간의 소리를 계산해 놓고, 네트워크 요청을 보내고, 응답받은 내용을 렌더링하며, 모니터에 페이지를 출력한다. 당신이 볼 때마다 컴퓨터는 작업을 잘 수행하고 있다. 당신이 인식하지 못할 만큼 짧은 시간동안 작업을 바꿔가면서, 컴퓨터는 몰래몰래 **동시** 작업을 하고 있다.

### 선점과 비선점

CPU는 연산 장치다. 데이터가 없다. 메모리(프로세스)는 데이터가 있지만 계산을 수행할 수 없다. 둘 모두 스스로 무언가를 판단할 수 없다. CPU는 메모리로부터 명령을 받아 실행한다. 스스로 무엇을 실행할지 결정할 수 없다. 메모리는 무엇을 실행할지 알고 있지만 실제로 실행할 수 없다. 때문에, 프로세스가 일단 CPU를 점거하면 독재가 가능하다. 프로세스가 가진 명령이 다른 프로세스를 모두 무시하고 영원히 자신만 실행하는 것이라면 운영체제도 개입할 수 없다.(운영체제 역시 프로세스다.) 이 같은 방식을 **비선점(non-preemptive)**라고 한다. CPU라는 자원을 선점(빼앗는)하지 못한다는 뜻이다. 여러 프로세스가 CPU를 사용하길 기다리고 있다. 어느 한 프로세스가 CPU를 사용중이면 다른 프로세스는 실행중인 프로세스가 끝나길 **기다려야 한다**. 

독재정치는 괴롭다. 많은 시민들이 굶어죽는다(starving). 앞의 프로세스가 CPU를 오래 붙들고 있으면 뒤의 프로세스는 실행 기회가 오지 않는다. _이런 현상을 기아현상(starving)이라고 한다._ 프로세스들이 권리를 주장하며 새로운 제도를 만들었다. 민주정부 수립이다. 주기적으로 선거를 통해 정부를 새로 구성하듯이, CPU를 사용하는 프로세스도 주기적으로 교체된다. 이처럼 CPU 자원을 선점(빼앗을)할 수 있는 방식을 선점형(preemptive)이라고 부른다. 과정의 핵심은 [인터럽트](#interrupt)다.  

CPU와 메모리의 한계 때문에, 소프트웨어적 방법으로 프로세스를 교체할 수 없다. CPU를 점거한 프로세스가 교체를 거부하면 방법이 없다. 그래서 하드웨어적 방법으로 프로세스를 교체한다. 우선 CPU는 한 명령을 실행할 때마다 인터럽트가 있는지 확인하도록 한다. 인터럽트가 없으면 이어서 실행하고, 있으면 교체되어야 함을 인지한다. CPU는 커널에게 다음 프로세스를 받는다. 커널은 다음 프로세스가 실행할 수 있는 시간을 타이머에 설정한다. CPU는 컨텍스트 스위칭 이후에 인터럽트를 계속 확인하면서 다음 프로세스의 명령을 실행한다. 커널이 세팅한 시간이 지나면 타이머가 인터럽트를 발생시킨다. CPU는 인터럽트가 발생했으므로 동작을 중지하고 다시 커널로부터 다음 작업을 받는다. 무한 반복...  

사이좋게 프로세스가 CPU를 나눠갖고 있다. 평화가 오는 듯했으나 불만이 생기기 시작했다. 모두 똑같이 시간을 나눠 쓰다보니, 주어진 시간을 다 활용하지 못하는 프로세스가 생겼다. 1분을 줬으면 1분동안 CPU를 열심히 사용해야 하는데, 1초만 쓰고 59는 CPU랑 노닥거리는 프로세스가 있었다. 못된 운영체제는 가장 비싼 자원인 CPU가 노는 꼴을 볼 수 없었다. 커널이 가만 살펴보니 CPU가 놀고 있는 이유는 IO때문이었다. 디스크로부터 정보를 불러오거나 네트워크를 통해 정보를 주고받는 경우 등이었다. 이같은 IO장치는 CPU속도를 쫓아올 수 없다. CPU는 데이터가 있어야 일을 할 수 있으므로 IO장치를 기다리는데 시간을 다 쓰는 것이었다. 화가 난 커널은 IO요청을 하면 CPU를 반납하는 규정을 두었다. 프로세스가 네트워크 통신을 요청하면, 네트워크 통신을 실행하고 프로세스를 쫒아낸다. CPU를 계속 줘봤자 할 수 있는게 없으니, 네트워크 응답이 오면 알려줄게 하며 프로세스를 대기실로 쫓아내버린다. 다음 프로세스는 CPU를 이어받아 필요한 작업을 실행한다. 네트워크 응답이 도착하면 인터럽트가 발생하고 커널은 기다리던 프로세스를 깨운다. 기다리던 프로세스는 데이터가 가 도착했으므로 CPU를 받아 원하는 작업을 수행할 수 있다.

![ready-queue](https://user-images.githubusercontent.com/40727649/68072200-3bcaa900-fdc6-11e9-9e80-ccb282e270d4.PNG)
